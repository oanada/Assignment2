...
##  Using K-medoids (Partitioning around Medoids)
...
Next, we will use Agglomerative Clustering with complete method to determine the optimum number of clusters.  We will be using the elbow method and compare with the silhouette method.
...
# final_df$clusternum <- ahc_tree
# cluster1 <- subset(final_df, final_df$clusternum==1)
# cluster2 <- subset(final_df, final_df$clusternum==2)
# cluster3 <- subset(final_df, final_df$clusternum==3)
# cluster4 <- subset(final_df, final_dfclusternum==4)
# 
# summary(cluster1)
# summary(cluster2)
# summary(cluster3)
...
ahc_results <- final_df %>% mutate(cluster = ahc_tree) %>%
...
clusplot(myTrain, ahc_tree, color=TRUE, shade=TRUE, labels=2, lines=0, main= 'Census income segents')
