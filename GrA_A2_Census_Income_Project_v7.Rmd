...
#INPUT_file_path="C:/Users_Folders/YORK_MLcertificate/Assignment2/asecpub20csv/US_Census_2020.csv"
...
### Profile clusterings with PAM clustering and tsne visualization
...
p1 <- ggplot(data = data.frame(x = clustnum, y = elbsumofsq[-1]), 
  ...
  labs(x = "Num.of clusters", y = "Elbow Sum of Squares") +
...
Based on the silhouette width calculation for clusters ranging from 2 to 10, cluster 4 yields the highest value.  
Having selected k = 4, let's look at each cluster by doing a summary.

```{r echo=FALSE, message=FALSE, warnings=FALSE, fig.cap=""}
ahc_tree <- cutree(ahc_fit, k=4)
...
df1_out_hac<-df1
df1_out_hac$cluster<-pam_fit$clustering
...
nleaves(ahc_dend)  # number of leaves in tree
nnodes(ahc_dend)  # number of nodes (=leaves + joins) in tree
plot(color_branches(ahc_dend, k=5),leaflab="none")
clusplot(df1, ahc_tree, color=TRUE, shade=TRUE, labels=2, lines=0, main= 'Census income segments')
